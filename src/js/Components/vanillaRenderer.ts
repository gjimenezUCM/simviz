...
/**
 * It creates the table employed to compare cases in SimViz. It uses Handlebars to
...
export class VanillaRenderer {
  ...
  constructor(
    ...
    for (const [attName, attType] of Object.entries(allAtts)) {
      // Ignore attribute id because it appear in the table header
      if (attName === this.attId) continue;
  ...
  public updateSimilarityValue(
    ...
    if (color !== null) {
      simValueElem && simValueElem.parentElement
  ...
  private updateCaseInColumn(
    ...
    for (let att of Object.keys(this.simAtts)) {
      let comparatorRow = document.querySelectorAll(
        `tr[data-att-name=${att}] td`
      );
      let itemElement = this.createAttributeValueElement(
        att,
        caseContent[<keyof Object>att]
      );
  ...
  private populateWeights() {
    let weights = document.querySelectorAll(
      "#case-comparison-panel .att-weight"
    );
  ...
  private createAttributeValueElement(attName: string, attValue: any): string {
    ...
    if (this.simAtts[attName]) {
      result = TemplateManager.generateWithRegisteredTemplate(
  ...
  /**
   * Update the local similarity value for an attribute (by name)
   * @param attName Name of the attribute
  ...
  private updateLocalSimilarity(
    ...
    if (attName in this.simAtts) {
      ...
      let valueElem = document.querySelector(
    ...
    for (let link of taxonomyLinks) {
      link.addEventListener("click", (event) => {
        if (event.currentTarget) {
          let label = (<HTMLElement>event.currentTarget).getAttribute(
            "data-label"
          );
          if (label) {
