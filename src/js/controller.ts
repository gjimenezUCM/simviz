...
class Controller {
    ...
    /**
     * Init the controller with the casebase and the similarity data currently loaded
     * @param casebaseDAO The casebase DAO currently loaded
     * @param simData The similarity data currently loaded
     */
    init(casebaseDAO:CasebaseDAO, simData: SimilarityData | null) {
        let allAttributes = casebaseDAO.getAttributes();
        let simDescription = simData ? simData.similarityDescription : null;
        this.tableComponent = new CaseComparator(allAttributes, simDescription, casebaseDAO.getAttId());
        this.tableComponent.resetTable();
        this.casebaseDAO = casebaseDAO;
        this.caseIds = casebaseDAO.getIds();
        this.similarityPanel.init(casebaseDAO);
