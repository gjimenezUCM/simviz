...
async function initApp() {
  ...
  if (await datasetLoader.init("data/datasets.json")) {
    let datasetMenu: HTMLSelectElement | null = <HTMLSelectElement>(
  ...
  let pinButtons = document.querySelectorAll(
    "#case-comparison-panel button[data-item-id]"
  );
  // Suscribe to click events on pin buttons
  for (let btn of pinButtons) {
    btn.addEventListener("click", (event) => {
      if (event.currentTarget) {
        let itemId = (<HTMLElement>event.currentTarget).getAttribute(
          "data-item-id"
        );
        if (itemId) {
...
async function loadDataset(datasetLoader: DatasetLoader, datasetName: string) {
  let casebaseDAO: CasebaseDAO | null = await datasetLoader.getCasebaseDAO(
    datasetName
  );
  if (casebaseDAO) {
    theSimilarityDAO.init(
      datasetLoader.getSimilarityFunctionsForDataset(datasetName),
      casebaseDAO.getIds()
    );
